...
exports.getAllVisualAudioUser = catchAsync(async (req, res, next) => {
    ...
    const features = new APIFeatures(VisualAudio.find(), req.query).filter().sort().limitFields().paginate(); //page='int'&limit='int'
...
exports.getVisualAudioUser = catchAsync(async (req, res, next) => {
    const user = await VisualAudio.findById(req.params.id);
