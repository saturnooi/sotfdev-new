...
exports.getAllMovingServicesUser = catchAsync(async (req, res, next) => {
    ...
    const features = new APIFeatures(MovingServices.find(), req.query).filter().sort().limitFields().paginate(); //page='int'&limit='int'
...
exports.getMovingServicesUser = catchAsync(async (req, res, next) => {
    const user = await MovingServices.findById(req.params.id);
